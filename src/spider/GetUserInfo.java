...
//        url="http://weibo.com/p/100505"+userid+"/info?mod=pedit_more";
        urls=new ArrayList<String>(){{
            add("http://weibo.com/p/100505"+userid+"/info?mod=pedit_more");
            add("http://weibo.com/p/100306"+userid+"/info?mod=pedit_more");
        }};
        String chtml = null;
//        String curl = null;
        for (String url : urls) {
            chtml = Spider.getHtml(url);
            if (Jsoup.parse(chtml).getElementsByAttributeValue("class", "li_1 clearfix").first()!= null) {
//                curl = url;
                break;
            }
        }
